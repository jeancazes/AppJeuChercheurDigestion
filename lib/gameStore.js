...
const createGameStore = () => {
  ...
  return {
    ...
    addResource: (resource) => {
      const newId = 'r' + (data.resources.length + 1) + '_' + Date.now();
      const newResource = {
        id: newId,
    ...
    updateResource: (resourceId, updates) => {
      ...
      data.resources[resourceIndex] = { ...data.resources[resourceIndex], ...updates };
      notify();
      return true;
    },
    ...
    deleteResource: (resourceId) => {
      ...
      // Retirer des achats de toutes les équipes
      data.equipes.forEach(equipe => {
        equipe.purchasedResources = equipe.purchasedResources.filter(id => id !== resourceId);
      });
    ...
    deleteClass: (classeId) => {
      ...
      // Supprimer aussi les équipes de cette classe
