...
const createGameStore = () => {
  ...
  const purchaseResource = async (equipeId, resourceId) => {
    try {
      ...
      // Double vérification : Lire le budget actuel de la DB
      ...
      // Vérifier à nouveau avec le budget réel de la DB
      if (resource.price > teamData.budget) {
        return { success: false, error: 'Budget insuffisant (vérification DB)' };
      }

      const newBudget = teamData.budget - resource.price;

      // Vérifier que le nouveau budget n'est pas négatif
      if (newBudget < 0) {
        return { success: false, error: 'Budget insuffisant (résultat négatif)' };
      }
      ...
      if (insertError) {
        // Si erreur UNIQUE violation, la ressource est déjà achetée
      ...
      // Déduire du budget avec vérification supplémentaire
      const { error: updateError } = await supabase
        .from('teams')
        .update({ budget: newBudget })
        .eq('id', equipeId)
        .gte('budget', resource.price); // Condition supplémentaire : budget doit être >= prix
  ...
  const getMemberSessions = async (equipeId) => {
    try {
      ...
      console.error('Erreur getMemberSessions:', error);
      return [];
    }
  };
  ...
  const updateMemberSession = async (equipeId, memberName, sessionNumber, grade) => {
    try {
      const sessionColumn = `session_${sessionNumber}`;
      ...
      console.error('Erreur updateMemberSession:', error);
      return false;
    }
  };
