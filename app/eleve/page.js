...
// PAGE FICHE ÉQUIPE
// ============================================
const FichePage = ({ equipe, classeInfo, onBack }) => {
  const levelInfo = LEVEL_CONFIG[equipe.level];
  const avatars = ['👨‍⚕️', '👩‍⚕️', '🧑‍🔬', '👨‍🔬', '👩‍🔬', '🥼', '🔬', '🧬'];
      ...
      <div style={{ padding: '20px' }}>
        <div style={{ background: COLORS.white, borderRadius: '24px', padding: '24px', boxShadow: `0 10px 40px ${COLORS.cardShadow}`, border: `3px solid ${COLORS.cardBorder}`, textAlign: 'center' }}>
          ...
          <div style={{ marginBottom: '20px' }}>
            <h3 style={{ color: COLORS.textLight, fontSize: '0.85rem', marginBottom: '10px' }}>Membres de l'équipe</h3>
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px', justifyContent: 'center' }}>
              {equipe.membres.map((membre, i) => (<span key={i} style={{ background: COLORS.secondary, color: COLORS.primaryDark, padding: '8px 14px', borderRadius: '12px', fontSize: '0.9rem', fontWeight: '600' }}>{membre}</span>))}
            </div>
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '12px' }}>
...
export default function StudentApp() {
  ...
  return (
    <div style={{ maxWidth: '500px', margin: '0 auto', minHeight: '100vh', fontFamily: "'Nunito', 'Segoe UI', sans-serif", position: 'relative', boxShadow: '0 0 60px rgba(0,0,0,0.1)' }}>
      ...
      {page === 'fiche' && <FichePage equipe={equipe} classeInfo={classeInfo} onBack={() => setPage('home')} />}
